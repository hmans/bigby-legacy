...
export class OrbitControls {
  ...
  zoom(scale: number): void {
    ...
    vec3.scale(
      ...
      this._camera.position,
      scale *
        (Math.min(this.maxRadius, Math.max(this.minRadius, radius * scale)) /
          (radius * scale))
    )
  ...
  orbit(deltaX: number, deltaY: number): void {
    ...
    const phi =
      Math.min(
        this.maxPhi,
        Math.max(this.minPhi, Math.acos(offset[1] / radius) - deltaPhi)
      ) || Number.EPSILON
    const theta =
      Math.min(
        this.maxTheta,
        Math.max(this.minTheta, Math.atan2(offset[2], offset[0]) + deltaTheta)
      ) || Number.EPSILON
  ...
  private _onPointerMove(event: PointerEvent): void {
    ...
    if (prevPointer) {
      ...
      const type =
        event.pointerType === "touch" ? this._pointers.size : event.buttons
      if (type === BUTTONS.LEFT) this.orbit(deltaX, deltaY)
      else if (type === BUTTONS.RIGHT && this.enablePan)
  ...
  private _onPointerUp(event: PointerEvent): void {
    this._element!.style.touchAction =
      this.enableZoom || this.enablePan ? "none" : "pinch-zoom"
